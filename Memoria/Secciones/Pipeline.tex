...
Este modelo, a diferencia de otros, requiere que los datos de entrada estén normalizados, por lo que se guardaron tanto el modelo y su \emph{scaler} en formato pkl para poder usarlos en el pipeline y así realizar las predicciones. 
...
Al comprobar los resultados de los métodos de ia explicable vemos que la técnica de explicabilidad SHAP global refleja que KNN asigna una alta importancia a la característica \emph{"spectral flatness"}, un patrón consistente con los resultados obtenidos en la fase de metodología (ver \ref{fig:pipe_knn_shap_global}). Pero al evaluar localmente con shap cada muestra vemos que la que detecta como generada correctamente la característica más predominante es el \emph{"spectral flatness"}, lo que corrobora lo visto anteriormente en los resultados, sin embargo, cuando vemos la imagen de la muestra que clasifica como generada erróneamente nos damos cuenta que la característica que más influye no es el \emph{"spectral flatness"} si no que lo son el \emph{"silence ratio"} y el \emph{"mfcc_1_mean"} (ver \ref{fig:pipe_knn_shap_local_1} y \ref{fig:pipe_knn_shap_local_2}) y por ello no actúa cómo actuaba con los otros audios.

Esto mismo se puede ver reflejado en las gráficas de lime \ref{fig:pipe_knn_lime_1} y \ref{fig:pipe_knn_lime_2}. 

Por último, vemos como Dice genera 3 instancias contrafactuales que indican qué alteraciones mínimas en las características analizadas transformarían la clasificación de un audio generado a uno real. Como se aprecia en la \autoref{tab:pipe_dice_knn}, estas modificaciones se concentran de manera prioritaria en ajustar los valores de \emph{"spectral flatness"}.

\section{Regresión Logística}

Este modelo, al igual que en KNN y a diferencia de otros, requiere que los datos de entrada estén normalizados, así que se guardaron el modelo y su \emph{scaler} en formato pkl para poder usarlos en el pipeline y así realizar las predicciones. 

Al resliazar las predicciones vemos como, al igual que en knn, los dos audios españoles los clasifica como generados.

Al comprobar los resultados de los métodos de ia explicable vemos que la técnica de explicabilidad SHAP global refleja que la Regresión Logística asigna una alta importancia a la característica \emph{"spectral flatness"},exactamente igual que ocurría en el knn (ver \ref{fig:pipe_lr_shap_global}). Por otro lado al evaluar localmente con shap cada muestra vemos que en la que detecta como generada correctamente la característica más predominante es el \emph{"spectral flatness"}, lo que corrobora lo visto anteriormente en los resultados. Sin embargo, cuando vemos la imagen de la muestra que clasifica como generada erróneamente nos damos cuenta de que, al igual que pasaba en KNN, la característica que más influye no es el \emph{"spectral flatness"} sino que lo son el \emph{"silence ratio"} y el \emph{"mfcc_1_mean"} con valores más altos que los vistos en knn, aún así vemos que en este modelo aparece la \emph{"kurosis"} intentando decantar la balanza para el otro lado por encima del \emph{"spectral flatness"} de nuevo (ver \ref{fig:pipe_lr_shap_local_1} y \ref{fig:pipe_lr_shap_local_2}).

Esto mismo se puede ver reflejado en las gráficas de lime \ref{fig:pipe_lr_lime_1} y \ref{fig:pipe_lr_lime_2}. 

Por último, se observa cómo DiCE genera tres instancias contrafactuales que indican las alteraciones mínimas requeridas en las características analizadas para transformar la clasificación de un audio generado a uno real. Como se aprecia en la \autoref{tab:pipe_dice_lr}, estas modificaciones se concentran de manera prioritaria en ajustar los valores de \emph{"spectral flatness"}, siendo en este modelo la única característica capaz de alterar la predicción final.
