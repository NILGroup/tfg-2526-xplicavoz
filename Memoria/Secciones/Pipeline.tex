...
Una vez cargado, se usan las funciones cargadas del archivo German/funciones\_aasist.py que contiene todas las funciones necesarias para poder aplicar el modelo y las distintas técnicas de XAI. Primero, se ejecuta la función ``\texttt{score\_audio}'' que devuelve los resultados en forma de diccionario con cuatro claves: ``logits'', un tensor que contiene los logit para ambas clases; ``score\_bonafide'', el valor de logits para la clase \textit{bona fide}; ``prob\_bonafide'', la probabilidad de que el audio sea \textit{bona fide} calculada a partir del logit, y ``pred\_clase'', que muestra la clase predicha por el modelo.
