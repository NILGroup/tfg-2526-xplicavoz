...
El motivo de probar un MLP (perceptrón multicapa) para esta tarea de clasificación fue, sobre todo, explorar qué posibles opciones tenemos a la hora de abordarla.

La idea inicial fue comprobar qué conseguíamos con los modelos más simples, ya que contamos con un dataset muy reducido.

El enlace al código se encuentra en el pie de página:\footnote{El código realizado para implementar este modelo se encuentra en: ``\href{https://github.com/NILGroup/tfg-2526-xplicavoz/blob/main/Modelos/Ivan/MLP.ipynb}{https://github.com/NILGroup/tfg-2526-xplicavoz/blob/main/Modelos/Ivan/MLP.ipynb}''}
...
El proceso de tratamiento de datos comienza extrayendo del dataset las características globales de los audios, es decir, los que representan una media. También se obtendrán los espectrogramas de cada audio. A continuación, emplearemos un MLP y una CNN(red neuronal convolucional) de PyTorch, que reciben como entrada tensores de datos. El objetivo de estas dos redes auxiliares es obtener vectores de \textit{embeddings} a partir de los audios.

\subsubsection*{Embeddings de los espectrogramas}

Para sacar los \textit{embeddings} de los espectrogramas emplearemos una CNN de la librería de \textit{Pytorch.nn} y \textit{Pytorch.nn.functional}.
...
\begin{itemize}
	...
	\item \textbf{Primera capa de pooling}: MaxPool2d(kernel\_size = 2, stride = 2), donde:
	  \begin{itemize}
...
\begin{enumerate}
 ...
 \item Optimizer = Adam (Algoritmo empleado para entrenar redes neuronales combinando el momentum y tasas de aprendizajes adaptativas).
\end{enumerate}