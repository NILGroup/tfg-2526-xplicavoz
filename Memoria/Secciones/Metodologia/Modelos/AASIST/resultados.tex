\footnote{Enlace al notebook de aplicación de XAIs: \url{https://github.com/NILGroup/tfg-2526-xplicavoz/blob/main/Modelos/German/AASIST_XAI_aplicacion.ipynb}}
...
En esta primera~\autoref{fig:AASIST_global_occlusion_temporal_logit}, vemos cómo ayuda cada tramo a los logits de los audios de cada clase. En lo referido a la clase \textit{bona}, podemos destacar tres fases:
\begin{itemize}
...
Sobre la clase \textit{spoof}, tenemos una gráfica bastante distinta con 4 regiones:
\begin{itemize}
    \item Entre \textbf{0,15 y 0,3} segundos, un pico positivo muy superior al resto de la zona.
    \item Entre \textbf{0,3 y 1,0} segundos, al igual que con la clase \textit{bona}, una zona con valores claramente negativos que indican la misma conclusión.
    \item Entre \textbf{1,2-1,4 y 1,5-1,8} segundos, volvemos a ver picos positivos que empujan el logit hacia la clase \textit{spoof}.
    \item Entre \textbf{3,8 y 4,0} segundos, una región negativa claramente diferenciada. Sin embargo, la precaución explicada para la clase \textit{bona} también se aplica aquí.
\end{itemize}

En resumen, la~\autoref{fig:AASIST_global_occlusion_temporal_margin} sugiere que el modelo se apoya en tramos concretos y no en toda la señal. En la clase \textit{bona}, la evidencia está muy localizada en la zona central, mientras que la clase \textit{spoof} usa evidencia menos estable y seguramente más basada en patrones o irregularidades locales.

Para interpretar esta gráfica, conviene recordar que al mostrar la variación del margen entre clases, las zonas positivas son segmentos \textit{pro-spoof} y las negativas, \textit{pro-bona}. Esta aclaración se aplica para ambas curvas, ya que para ambos casos calculamos el margen como la diferencia entre la salida de la clase \textit{spoof} y la de la clase \textit{bona}.
\begin{figure}
    \centering
...
En cuanto a las regiones más relevantes, podemos señalar dos: la primera, comprendida entre \textbf{0,2 y 0,3}, es pro-spoof, especialmente la curva \textit{spoof}. Esto significa que el modelo encuentra evidencia que le empuja la decisión hacia \textit{spoof}. La segunda región se encuentra entre \textbf{1,0 y 2,0} segundos, la cual empuja la decisión claramente a \textit{bona}.
Sobre el patrón general de la gráfica, podemos intuir que los audios \textit{spoof} parecen naturales la mayor parte del tiempo, ya que la mayoría de valores son negativos. Sin embargo, AASIST debe encontrar patrones muy localizados que le ayudan a clasificar los audios como spoof.

En la~\autoref{fig:AASIST_global_occlusion_frecuencial_logit}, podemos observar claramente cómo la banda 0-500 Hz es muy dominante, e incluso el tramo 500-1000 Hz en \textit{bonas}. Esto es coherente con lo que uno espera de la voz humana, ya que en las bajas frecuencias se concentra no solo la frecuencia fundamental, habitualmente situada en torno a 80-300 Hz, sino también una parte importante de la energía global de la señal y de su estructura armónica.
...
En la~\autoref{fig:AASIST_global_occlusion_frecuencial_margin} podemos destacar tres regiones:
\begin{itemize}
...
Antes de empezar a analizar la~\autoref{fig:AASIST_global_LIG_mapas}, cabe recordar que mostramos la magnitud absoluta de atribución, no signo, por lo que solo podemos destacar relevancia espectral, pero no si una región empuja a cierta clase.

Interpretando los comportamientos, por un lado, observamos actividad predominantemente focalizada en los filtros de baja frecuencia en el mapa de \textit{bonas}. Por otro lado, en el mapa de \textit{spoofs}, aunque también observamos esta predominancia, se observa una actividad algo mayor en las frecuencias altas. Esto podría reforzar la idea mencionada de patrones artificiales o irregularidades espectrales en frecuencias superiores.
\begin{figure}
    \centering
...
En ambas gráficas de la~\autoref{fig:AASIST_global_LIG_mapas_norm}, seguimos observando actividad casi continua de los filtros de baja frecuencia en los 4 segundos. Respecto al mapa de \textit{bonas}, destaca la región entre \textbf{1 y 2} segundos, donde hay más actividad. Esto coincide con lo mencionado en la gráfica temporal con logits de Occlusion.

En el mapa de \textit{spoofs}, vemos una actividad más fragmentada y localizada en eventos temporales concretos, como, por ejemplo, entre los segundos \textbf{1 y 1,5}, y entre los segundos \textbf{3,6 y 3,9}. Ambas regiones también coinciden con las conclusiones extraídas en Occlusion temporal.
\begin{figure}
    \centering
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
...
En la~\autoref{fig:AASIST_global_IG_audio}, volvemos a ver el patrón en el que entre los segundos \textbf{1 y 1,8} hay una región importante para ambas clases. Posteriormente, entre los segundos \textbf{2 y 3,5}, la atribución es menor, y finalmente, sobre el segundo \textbf{3,9}, un pico de relevancia de la curva \textit{bona} claramente diferenciable. Este patrón refuerza la interpretación obtenida en otras gráficas temporales.
\begin{figure}
    \centering
...
Analizando la~\autoref{fig:AASIST_global_RISE}, en conjunto, ambas gráficas vuelven a reforzar lo comentado sobre cómo regiones concretas influyen en mayor medida sobre el logit. Sin embargo, algo interesante de las gráficas de esta figura es que hay menos picos que en las gráficas de Occlusion, lo que sugiere que aquí se capta una relevancia más repartida por la señal.

Observando qué curva presenta valores superiores, nos damos cuenta de que en la que representa el logit, la clase \textit{bona} tiene mayor fuerza, mientras que en la que representa el \textit{margen}, ocurre lo contrario. Esto se puede deber a que en audios de la clase \textit{bona}, hay una evidencia relativamente estable que sostiene el logit de la clase durante el audio, mientras que en audios de la clase \textit{spoofs}, hay regiones que no necesariamente maximizan el logit de forma tan uniforme, pero que, sin embargo, sí resultan importantes a la hora de separar la muestra respecto a \textit{bona}.
\begin{figure}
    \centering
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
...
Antes de analizar la~\autoref{fig:AASIST_global_SHAP}, conviene recordar que estas figuras no representan explicaciones obtenidas directamente sobre AASIST, sino sobre modelos \textit{surrogate}. Por ello, su interpretación debe entenderse como una explicación complementaria: no describen de forma interna el funcionamiento de AASIST, pero sí permiten identificar qué rasgos acústicos interpretables resultan más influyentes en modelos que aproximan su comportamiento.

En cuanto a las figuras, la de la izquierda corresponde al modelo clasificador y la de la derecha al modelo regresor. Consideradas en conjunto, ambas muestran un patrón bastante coherente. La característica más influyente, especialmente en el clasificador, es \textbf{RMS}, es decir, la energía media del audio. Junto a ella, también destacan dos familias de variables especialmente relevantes: los \textbf{formantes} y los \textbf{MFCC}. La importancia observada en las frecuencias bajas y bajas-medias parece estar asociada, al menos en parte, a la estructura formántica y a la envolvente espectral de la voz, rasgos estrechamente relacionados con la naturalidad del habla. En este sentido, la importancia de variables como \textit{f1\_mean}, \textit{f2\_mean} o \textit{f3\_mean} refuerza la atribución que otras técnicas XAI ya sugerían para estas regiones del espectro.
