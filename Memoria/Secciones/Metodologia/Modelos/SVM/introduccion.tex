...
\begin{itemize}
	\item \textbf{kernel (``linear'')}: La función matemática que transforma los datos para encontrar el hiperplano óptimo.
	\item \textbf{C}: Parámetro de regularización.
	\item \textbf{probability}: Habilita el cálculo de probabilidades en lugar de solo clases.
	\item \textbf{random\_state}: Fija el estado aleatorio para reproducibilidad.
	\item \textbf{class\_weight}: Ajuste de pesos para clases desbalanceadas inversamente proporcionales a la frecuencia de cada clase.
\end{itemize}
...
\begin{itemize}
	\item \textbf{kernel (``rbf'')}: La función matemática que transforma los datos para encontrar el hiperplano óptimo.
	\item \textbf{gamma}: Parámetro que controla la influencia de cada punto de entrenamiento en la frontera de decisión.
\end{itemize}
